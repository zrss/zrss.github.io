...
3. **FlashInfer cubin 预编译进容器镜像**：build 时用 `flashinfer download-cubin` 烘焙，消除运行时编译
...
| FlashInfer cubin 烘进镜像 | 消除最大头的运行时编译 |
...
存储和启动路径也应该平台化。镜像、JIT cache、checkpoint 都走“本地热路径 + 共享持久化路径”：节点本地 NVMe 承担高频读写，共享存储只做少并发归档；checkpoint 异步化，save plan 缓存化；JIT artifact 可以通过 DaemonSet 预热到节点，或者在镜像构建阶段烘焙进 runtime layer。
