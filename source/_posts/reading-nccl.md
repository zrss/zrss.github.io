...
这篇主要记录 NCCL 2.14.3 中 `net_ib` 和 RDMA 相关细节。关于较新 NCCL `master` 分支中 communicator 建链、transport 选择、`ncclConnInfo` 写入，以及建链后一次通信如何执行，可以参考：[NCCL 从建链到通信](/archives/9e7a43b2.html)。

和新文的对应关系可以粗略理解为：

```text
NCCL 从建链到通信：
  解释较新 NCCL master 主流程中如何选择 NET transport、生成 ncclConnInfo、投递 proxy op。
...
1. 通信算子 entry 加入 ncclTask 队列
1. group 提交通信算子 entry 到 proxy
1. proxy 使用通信算子对应的具体实现来完成参数传递
